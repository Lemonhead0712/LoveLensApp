...
import { analyzeScreenshots } from "@/lib/analyze-screenshots-enhanced"
...
export function UploadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        analysisResult = await analyzeScreenshots(base64Files)
        setProgress(80)
      } catch (analysisError) {
        // Check if error is related to OCR
        if ((analysisError as Error).message.includes("extract text")) {
          throw new Error("OCR_FAILED")
        }
        // Otherwise, it's a general analysis error
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      {isLoading ? (
        <LoadingScreen progress={progress} message="Analyzing your conversation..." />
      ) : (
