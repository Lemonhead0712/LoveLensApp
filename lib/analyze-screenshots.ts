...
import { processImageForConversation } from "./ocr-service"
import { analyzeEmotionalIntelligence } from "./gpt-ei-service"
...
/**
 * Analyzes screenshots to extract messages and perform sentiment analysis
 *
...
/**
 * Analyzes screenshots to extract and analyze conversation
 * @param imageData The image data as a base64 string
 * @returns Analysis results for both participants
 */
export async function analyzeScreenshots(imageData: string) {
  try {
    // Extract conversation with position information
    const conversation = await processImageForConversation(imageData)

    if (!conversation.messages || conversation.messages.length === 0) {
      throw new Error("No messages were extracted from the image")
    }

    // Split messages by position
    const leftMessages = conversation.messages.filter((m) => m.position === "left")
    const rightMessages = conversation.messages.filter((m) => m.position === "right")

    // Check if we have messages for both participants
    if (leftMessages.length === 0 || rightMessages.length === 0) {
      console.warn("Missing messages for one participant", {
    ...
    const leftAnalysis = await analyzeParticipantMessages(leftMessages, "Person A")
    const rightAnalysis = await analyzeParticipantMessages(rightMessages, "Person B")

    return {
      conversation,
      analysis: {
        personA: leftAnalysis,
        personB: rightAnalysis,
        overall: {
          // Overall relationship dynamics can still be computed
          compatibility: calculateCompatibility(leftAnalysis, rightAnalysis),
          communicationPatterns: identifyCommunicationPatterns(leftMessages, rightMessages),
        },
      },
    }
  } catch (error) {
...
/**
 * Analyzes messages for a single participant
 * @param messages The messages from one participant
 * @param participantName The name to use for this participant
 * @returns Analysis results for this participant
 */
async function analyzeParticipantMessages(messages: Message[], participantName: string) {
  if (!messages.length) {
    return {
  ...
  const analysis = await analyzeEmotionalIntelligence(messageText, participantName)
 ...
 * @param analysisA Analysis results for person A
 * @param analysisB Analysis results for person B
 * @returns Compatibility score and factors
 */
function calculateCompatibility(analysisA: any, analysisB: any) {
  // Implement compatibility calculation logic
  // This is a placeholder implementation
  const eiCompatibility = calculateEICompatibility(analysisA.emotionalIntelligence, analysisB.emotionalIntelligence)

  return {
    score: eiCompatibility.score,
    factors: eiCompatibility.factors,
  }
}

/**
 * Calculates emotional intelligence compatibility
 * @param eiA Emotional intelligence scores for person A
 * @param eiB Emotional intelligence scores for person B
 * @returns Compatibility score and factors
 */
function calculateEICompatibility(eiA: any, eiB: any) {
  // Calculate average difference across EI dimensions
  ...
  dimensions.forEach((dim) => {
    const diff = Math.abs(eiA[dim] - eiB[dim])
    totalDiff += diff

    // Identify complementary strengths
    if (eiA[dim] > 7 && eiB[dim] < 5) {
      factors.push(`Person A's strong ${formatDimension(dim)} complements Person B`)
    } else if (eiB[dim] > 7 && eiA[dim] < 5) {
      factors.push(`Person B's strong ${formatDimension(dim)} complements Person A`)
    }

    // Identify shared strengths
    if (eiA[dim] > 7 && eiB[dim] > 7) {
      factors.push(`Both share strong ${formatDimension(dim)}`)
    }
  })

  // Calculate score (10 - average difference)
  const avgDiff = totalDiff / dimensions.length
  const score = Math.max(0, Math.min(10, 10 - avgDiff))

  return {
    score,
    factors: factors.length ? factors : ["Insufficient data for detailed compatibility factors"],
  }
}

/**
 * Formats dimension name for readability
 * @param dimension The dimension name in camelCase
 * @returns Formatted dimension name
 */
function formatDimension(dimension: string): string {
  const formatted = dimension.replace(/([A-Z])/g, " $1").toLowerCase()

  return formatted.charAt(0).toUpperCase() + formatted.slice(1)
}

/**
 * Identifies communication patterns between participants
 * @param messagesA Messages from person A
 * @param messagesB Messages from person B
 * @returns Communication patterns
 */
function identifyCommunicationPatterns(messagesA: Message[], messagesB: Message[]) {
  // Implement communication pattern identification logic
  // This is a placeholder implementation
  return {
    responseTime: "Normal",
    messageLength: compareMessageLengths(messagesA, messagesB),
    initiationBalance: calculateInitiationBalance(messagesA, messagesB),
  }
}

/**
 * Compares message lengths between participants
 * @param messagesA Messages from person A
 * @param messagesB Messages from person B
 * @returns Message length comparison
 */
function compareMessageLengths(messagesA: Message[], messagesB: Message[]) {
  if (!messagesA.length || !messagesB.length) return "Unknown"

  const avgLengthA = messagesA.reduce((sum, m) => sum + m.text.length, 0) / messagesA.length
  const avgLengthB = messagesB.reduce((sum, m) => sum + m.text.length, 0) / messagesB.length

  const ratio = avgLengthA / avgLengthB

  if (ratio > 2) return "Person A sends much longer messages"
  if (ratio < 0.5) return "Person B sends much longer messages"
  return "Both send similar length messages"
}

/**
 * Calculates initiation balance between participants
 * @param messagesA Messages from person A
 * @param messagesB Messages from person B
 * @returns Initiation balance
 */
function calculateInitiationBalance(messagesA: Message[], messagesB: Message[]) {
  // This is a simplified implementation
  // In a real implementation, we would need timestamps to determine who initiates conversations
  return "Cannot determine without timestamps"
}
